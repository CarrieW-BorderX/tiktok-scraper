...
def scrape_tiktok_user_videos(user, search_type="username", max_videos=2000, batch_size=50, rest_seconds=5, user_data_dir=None, retry_delay=2, max_retries=5):
    """
    Scrapes TikTok video URLs from a user's page (by username or user ID) in batches with rest intervals.
    Only refreshes the page if the scroll reaches the bottom and no new videos are loaded.
    """
    base_url = f"https://www.tiktok.com/@{user}"
...
def scrape_tiktok_hashtag_videos(hashtag, max_videos=2000, batch_size=50, rest_seconds=5, user_data_dir=None, retry_delay=2, max_retries=5):
    """
